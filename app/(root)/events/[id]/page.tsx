...
const EventDetails = async ({ params: { id }, searchParams }: SearchParamProps) => {
  ...
  return (
    <>
        ...
        <div className="wrapper grid grid-cols-1 md:grid-cols-2 2xl:max-w-7xl">
          ...
          <div className="flex w-full flex-col gap-8 md:p-10">
            <div className="flex flex-col gap-4 md:gap-6">
              <h2 className='pt-5 md:pt-0 h3-bold'>{event.title}</h2>

              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div className="flex gap-3">
                  <p className="text-base font-bold md:p-bold-20 rounded-full bg-green-500/10 px-5 py-2 text-green-700">
                    {event.isFree ? 'FREE' : `₹${event.price}`}
                  </p>
                  <p className="text-base md:p-medium-16 rounded-full bg-grey-500/10 px-4 py-2.5 text-grey-500">
                    {event.category.name}
                  </p>
                </div>
            ...
            <div className="flex flex-col gap-2">
              <p className="text-lg font-bold text-grey-600">What You'll Learn:</p>
      ...
      <section className="wrapper my-8 flex flex-col gap-8 md:gap-10">
